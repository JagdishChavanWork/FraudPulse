...
def _render_filters(frame: pd.DataFrame) -> tuple[pd.DataFrame, list[str], tuple[float, float]]:
    ...
    filtered = apply_credit_filters(frame, approved_flags, [], income_range, age_range)
...
def render_credit_dashboard(show_header: bool = True) -> None:
    ...
    with portfolio_tab:
        ...
        with col4:
            st.plotly_chart(box_by_category(filtered, "approved_flag", "max_credit_amount", "Maximum Credit Amount by Approval Category", "Approval Category", "Maximum Credit Amount"), use_container_width=True)
            _chart_note("Shows how recommended exposure changes across approval categories.")

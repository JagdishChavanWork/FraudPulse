...
def count_bar(frame: pd.DataFrame, column: str, title: str) -> go.Figure:
...
def histogram(frame: pd.DataFrame, column: str, title: str, nbins: int = 30) -> go.Figure:
    fig = px.histogram(
        frame,
        x=column,
        nbins=nbins,
...
def box_by_category(frame: pd.DataFrame, x: str, y: str, title: str) -> go.Figure:
    fig = px.box(
        frame,
        x=x,
        y=y,
...
def scatter_segment(frame: pd.DataFrame, x: str, y: str, color: str, title: str) -> go.Figure:
    fig = px.scatter(
        frame,
        x=x,
        y=y,

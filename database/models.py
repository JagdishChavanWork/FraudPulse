...
class FraudDetectionData(Base):
    ...
    account_type: Mapped[str | None] = mapped_column(String(40), index=True)
    account_type_enc: Mapped[int | None] = mapped_column(Integer)
    city_tier: Mapped[int | None] = mapped_column(Integer, index=True)
    tenure_days: Mapped[float | None] = mapped_column(Float)
    account_balance: Mapped[float | None] = mapped_column(Float)
    avg_monthly_spend: Mapped[float | None] = mapped_column(Float)
    avg_txn_amount: Mapped[float | None] = mapped_column(Float)
    avg_txn_per_day: Mapped[float | None] = mapped_column(Float)
    usual_txn_hour: Mapped[int | None] = mapped_column(Integer)
    txn_type: Mapped[str | None] = mapped_column(String(40), index=True)
    txn_type_enc: Mapped[int | None] = mapped_column(Integer)
    txn_amount: Mapped[float | None] = mapped_column(Float)
    txn_hour: Mapped[int | None] = mapped_column(Integer, index=True)
    txn_day_of_week: Mapped[int | None] = mapped_column(Integer)
    is_weekend: Mapped[bool | None] = mapped_column(Boolean, index=True)
    amount_to_avg_ratio: Mapped[float | None] = mapped_column(Float, index=True)
    balance_drain_pct: Mapped[float | None] = mapped_column(Float, index=True)
    hour_anomaly: Mapped[bool | None] = mapped_column(Boolean, index=True)
    velocity_24hr: Mapped[int | None] = mapped_column(Integer, index=True)
    new_txn_type_flag: Mapped[bool | None] = mapped_column(Boolean, index=True)
    large_round_amt_flag: Mapped[bool | None] = mapped_column(Boolean, index=True)
    days_since_last_txn: Mapped[float | None] = mapped_column(Float)
    prior_fraud_complaint: Mapped[bool | None] = mapped_column(Boolean, index=True)
    fraud_type: Mapped[str | None] = mapped_column(String(80), index=True)
    age_group: Mapped[str | None] = mapped_column(String(40), index=True)
